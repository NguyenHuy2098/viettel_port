...
interface Props {
  ...
  suggestions: CommoditySuggestedItemInter[];
}
  ...
  return (
    <Typeahead
      ...
      options={map(props.suggestions, item => ({
        id: get(item, 'name') + get(item, 'goodsValue'),
        label: get(item, 'name'),
        goodsValue: get(item, 'goodsValue'),
        quantity: get(item, 'quantity'),
        commodityType: get(item, 'commodityType'),
